...
%note: this script ignores the sigmax, sigmay, sigmaxy, and tag values
...
while(ischar(l))
    ...
    bottle = sscanf(l, '%i %i %i %i %i %i');
    temp = length(bottle) / 6;
    vi = vi + length(bottle) / 6;
    bottle = reshape(bottle, 6, length(bottle)/6)';
