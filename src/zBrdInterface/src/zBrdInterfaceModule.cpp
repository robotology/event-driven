...
/* 
 * Copyright (C) 2010 RobotCub Consortium, European Commission FP6 Project IST-Authors
 * 004370: Francesco Rea
...
/**
 * @file zBrdInterfaceModule.cpp
 * @brief Implementation of the zBrdInterfaceModule (see header file).
 */

#include <iCub/zBrdInterfaceModule.h>
...
bool zBrdInterfaceModule::configure(yarp::os::ResourceFinder &rf) {
    ...
    */
    ...
    esThread=new zBrdInterfaceThread();
...
bool zBrdInterfaceModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool zBrdInterfaceModule::close() {
    handlerPort.close();
    /* stop the thread */
    //cfThread->stop();
...
bool zBrdInterfaceModule::respond(const Bottle& command, Bottle& reply) {
...
bool zBrdInterfaceModule::updateModule() {
    return true;
}
