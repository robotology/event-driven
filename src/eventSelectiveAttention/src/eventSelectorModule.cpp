...
/**
 * @file eventSelectorModule.cpp
 * @brief Implementation of the eventSelectorModule (see header file).
 */

#include <iCub/eventSelectorModule.h>
...
bool eventSelectorModule::configure(yarp::os::ResourceFinder &rf) {
...
bool eventSelectorModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool eventSelectorModule::close() {
    handlerPort.close();
    /* stop the thread */
    cfThread->stop();
...
bool eventSelectorModule::respond(const Bottle& command, Bottle& reply) {
...
bool eventSelectorModule::updateModule() {
    return true;
}
