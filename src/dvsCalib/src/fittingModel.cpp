...
#define N 1000
#define NBR_PARAMETERS 4
#define ERROR_FACTOR 10.0
...
double thetasOfPolarCoord[N];
bool modelReady = false;
void setThetasOfPolarCoord(double* valueArray){
    for(int i=0; i<N ; ++i){
        thetasOfPolarCoord[i] = valueArray[i];
    }
    modelReady = true;
}
int modelFunction(const gsl_vector * x, void *dataE, gsl_vector * f)
{
    ...
    double K1 = gsl_vector_get (x, 2);
    double K2 = gsl_vector_get (x, 3);
    ...
    for (i = 0; i < n; i++)
     {
       ...
       double t = i;
       double z = r0*cos(thetasOfPolarCoord[i] - phi);
    ...
    for (size_t i = 0; i < n; i++)
     {
       ...
       double t = i;
       double s = sigma[i];
       double cosineOfDiff = cos(thetasOfPolarCoord[i] - phi);
       double z = r0*cosineOfDiff;
       double delFbydelZ = (1.0 + 3.0*K1*z*z + 5.0*K2*z*z*z*z);
       gsl_matrix_set (J, i, 0, ERROR_FACTOR*delFbydelZ*cosineOfDiff); 
       gsl_matrix_set (J, i, 1, ERROR_FACTOR*delFbydelZ*r0*sin(thetasOfPolarCoord[i] - phi));
       gsl_matrix_set (J, i, 2, ERROR_FACTOR*z*z*z);
       gsl_matrix_set (J, i, 2, ERROR_FACTOR*z*z*z*z*z);
     }
    return GSL_SUCCESS;
}
