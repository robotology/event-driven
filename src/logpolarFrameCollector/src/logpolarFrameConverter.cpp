...
void lFrameConverter::reset() {
    memset(converterBuffer_copy,0,BUFFERDIM);
}

void lFrameConverter::copyChunk(char* bufferCopy) {        
    mutex.wait();
...
void lFrameConverter::onRead(eventBuffer& i_ub) {
    valid = true;
    //printf("onRead ");
...
void lFrameConverter::resetTimestamps() {
    //unmask_events.resetTimestamps();
}

void lFrameConverter::getMonoImage(ImageOf<PixelMono>* image, unsigned long minCount, unsigned long maxCount, bool camera){
    assert(image!=0);
...
unsigned long lFrameConverter::getLastTimeStamp() {
    //return unmask_events.getLastTimestamp();
    return 0;
}

unsigned long lFrameConverter::getLastTimeStampRight() {
    //return unmask_events.getLastTimestampRight();
    return 0;
}

unsigned long lFrameConverter::getEldestTimeStamp() {
    //return unmask_events.getEldestTimeStamp();
    return 0;
}

void lFrameConverter::clearMonoImage() {
    //unmask_events.cleanEventBuffer();
}

lFrameConverter::~lFrameConverter() {
