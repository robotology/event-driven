...
/**********************************************************/
void circleDisparity::onRead(eventdriven::vBottle &vBottleIn)
{
    yarp::os::Stamp st;
    this->getEnvelope(st);

    eventdriven::vBottle &vBottleOut = outPort.prepare();
    vBottleOut = vBottleIn;

    //we just need to get our updated TS
    eventdriven::vQueue q = vBottleIn.getAll();
    q.sort(true);
    ...
    eventdriven::vQueue fq;
    std::vector<int> xs, ys, rs;
    eventdriven::ClusterEventGauss circleft;
    eventdriven::ClusterEventGauss circright;
    eventdriven::ClusterEventGauss *circc;

    //update the FIFOs
    for(unsigned int i = 0; i < q.size(); i++) {
        eventdriven::ClusterEventGauss * vc = q[i]->getUnsafe<eventdriven::ClusterEventGauss>();
        if(vc->getChannel()) {
            FIFOc = &FIFOR;
            circc = &circright;
        } else {
        ...
        for(int i = 0; i < n; i++) {
            eventdriven::ClusterEventGauss *vtw = fq[i]->getUnsafe<eventdriven::ClusterEventGauss>();
            xs[i] = vtw->getXCog();
            ys[i] = vtw->getYCog();
            rs[i] = vtw->getXSigma2();
