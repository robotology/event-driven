...
/**
 * @file eventSnifferModule.cpp
 * @brief Implementation of the eventSnifferModule (see header file).
 */

#include <iCub/eventSnifferModule.h>
...
bool eventSnifferModule::configure(yarp::os::ResourceFinder &rf) {
...
bool eventSnifferModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool eventSnifferModule::close() {
    handlerPort.close();
    /* stop the thread */
    //cfThread->stop();
...
bool eventSnifferModule::respond(const Bottle& command, Bottle& reply) {
...
bool eventSnifferModule::updateModule() {
    return true;
}
