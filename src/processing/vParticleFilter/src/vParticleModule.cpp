...
void drawEvents(yarp::sig::ImageOf< yarp::sig::PixelBgr> &image, emorph::vQueue &q, double tw = 0) {
...
vSurfaceHandler::vSurfaceHandler(unsigned int width, unsigned int height)
{
    ...
    surfaceLeft = emorph::temporalSurface(width, height);
    surfaceRight = emorph::temporalSurface(width, height);
    strict = false;
...
void vSurfaceHandler::onRead(emorph::vBottle &inputBottle)
{
    yarp::os::Stamp st;
    getEnvelope(st);
    ...
    //create event queue
    emorph::vQueue q = inputBottle.get<emorph::AddressEvent>();
    q.sort(true);
...
emorph::vQueue vSurfaceHandler::queryEvents(unsigned long int conditionTime, unsigned int temporalWindow)
{
...
//    }
    ...
    mutexsignal.lock();
    emorph::vQueue temp = surfaceLeft.getSurf_Tlim(tw);
    mutexsignal.unlock();
...
void particleProcessor::run()
{
    ...
    while(!isStopping()) {
        ...
        ptime = yarp::os::Time::now();
        emorph::vQueue stw = eventhandler.queryEvents(unwrap.currentTime() + rate, maxtw);
    ...
    for(int i = pStart; i < pEnd; i++) {
        for(unsigned int j = 0; j < (*stw).size(); j++) {
            if((*deltats)[j] < (*particles)[i].gettw()) {
                emorph::AddressEvent *v = (*stw)[j]->getUnsafe<emorph::AddressEvent>();
...
emorph::vQueue temporalSelection(emorph::vQueue &q, int ctime, int dtime)
{
    emorph::vQueue subq;

    if(ctime - dtime < 0) {
        for(unsigned int i = 0; i < q.size(); i++) {
            if(q[i]->getStamp() > ctime - dtime + emorph::vtsHelper::maxStamp() || q[i]->getStamp() < ctime)
                subq.push_back(q[i]);
        }
    } else {
        for(unsigned int i = 0; i < q.size(); i++) {
...
/******************************************************************************/
void vParticleReader::onRead(emorph::vBottle &inputBottle)
{
