...
unmaskDVS128::unmaskDVS128(uint _type, bool _save)
:save(_save)
{
    eventCounter=0;
    eventIndex=0;
...
unmaskDVS128::unmaskDVS128(const unmaskDVS128 &_obj)
{
    objcpy(_obj);
}

unmaskDVS128::~unmaskDVS128()
{
    delete[] buffer;
}

unmaskDVS128& unmaskDVS128::operator=(const unmaskDVS128 &_obj)
{
    objcpy(_obj);
    return *this;
}

void unmaskDVS128::objcpy(const unmaskDVS128 &_obj)
{
    if(this!=&_obj)
    {
...
void unmaskDVS128::setBuffer(char* i_buffer, uint i_sz)
{
    mutex.wait();
    ...
    {
#ifdef _DEBUG
        std::cout << "[unmaskDVS128] expand the size of the gBuffer" << std::endl;
#endif
        //szInMem+=BUFFERBLOCK;
    ...
    {
#ifdef _DEBUG
        std::cout << "[unmaskDVS128] expand the size and recopy of the gBuffer" << std::endl;
#endif
...
#ifdef _DEBUG
    std::cout << "[unmaskDVS128] concat the buffer (size in byte: " << szBuffer+i_sz << ", real size in mem: " << szInMem << ")" << std::endl;
#endif
...
void unmaskDVS128::reshapeBuffer()
{
    mutex.wait();
...
int unmaskDVS128::getUmaskedData(uint& addrx, uint& addry, int& polarity, uint& eye, uint& timestamp)
{
    int res=1;
    if((eventIndex+(typeOfRecord))>szBufSnapShot || szBufSnapShot==0)
    {
#ifdef _DEBUG
        std::cout << "[unmaskDVS128] make a snapshot of the buffer" << std::endl;
#endif
        if(!snapBuffer())
            return 0;
    }
#ifdef _DEBUG
    std::cout << "[unmaskDVS128] unmask data [" << eventIndex << " " << eventIndex+(typeOfRecord-1) << "] on " << szBufSnapShot << std::endl;
#endif
...
#ifdef _DEBUG
    std::cout << "[unmaskDVS128] buffer not empty (=> size: " << szBuffer << ", make the snapShot" << std::endl;
#endif
    delete[] bufSnapShot;
...
void unmaskDVS128::saveBuffer(char *_buf, uint _sz)
{
