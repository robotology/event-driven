...
unmask::unmask(const unmask &_obj)
{
    objcpy(_obj);
}
...
void unmask::objcpy(const unmask &_obj)
{
    if(this!=&_obj)
    {
...
unmask& unmask::operator=(const unmask &_obj)
{
    objcpy(_obj);
    return *this;
}

void unmask::unmaskEvent(uint evPU, uint& x, uint& y, int& pol, uint& eye)
{
