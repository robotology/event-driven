...
#include <iCub/logpolarRemapper.h>
#include <cassert>
...
logpolarRemapper::logpolarRemapper():convert_events(128,128) {
    retinalSize=128;
    outputWidth=320;
    outputHeight=240;
    unmask_events.start();
...
logpolarRemapper::~logpolarRemapper() {
    delete &unmask_events;
    delete &convert_events;
}

void logpolarRemapper::onRead(sendingBuffer& i_ub) {
...
void logpolarRemapper::getMonoImage(ImageOf<PixelMono>* image){
    assert(image!=0);
...
void logpolarRemapper::clearMonoImage() {
