...
/**
 * @file logAexGrabberModule.cpp
 * @brief Implementation of the logAexGrabberModule (see header file).
 */
...
bool logAexGrabberModule::configure(yarp::os::ResourceFinder &rf) {
...
bool logAexGrabberModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool logAexGrabberModule::close() {
    handlerPort.close();
    D2Y->stop();
    /* stop the thread */
    return true;
...
bool logAexGrabberModule::respond(const Bottle& command, Bottle& reply) {
    bool ok = false;
...
bool logAexGrabberModule::updateModule() {
    return true;
}

double logAexGrabberModule::getPeriod() {
