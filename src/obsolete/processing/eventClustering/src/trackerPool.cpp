...
void TrackerPool::update(emorph::ecodec::AddressEvent *ptr, int ev_t){
...
void TrackerPool::getMeanActivity(double &act){
    ...
    for(int ii=0; ii<trackers_.size(); ii++){
        ...
        trackers_[ii].getActivity(act_[ii]);
        act += act_[ii];
        
    }
    act = act/trackers_.size();
...
void TrackerPool::regulate_pool(int ts){
    ...
    // activity of each tracker, needed to change the activity threshold of the trackers
