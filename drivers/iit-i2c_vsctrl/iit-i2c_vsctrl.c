 ...
 */
// Use the FIFO write method when writing the BIAS. (More efficient, but may have problems with I2C not ready with slow devices, due to Linux I2C drivers
...
static int icub_vsctrl_init(struct icub_vsctrl_data *pdata, int chip_type) {
    ...
    if (chip_type == C_CHIP_DVS) {
        ...
        if (pdata->chip_type != chip_type) {
            ...
            icub_vsctrl_write_byte(pdata, C_BGCNFG0_ADDR, 0x09);  // BG clock and latch active low - Force the BG type to "SOFTWARE" to override the last 12 clock cycle problem
#else
        ...
        icub_vsctrl_write_byte(pdata, C_SRCTIM1_ADDR, 1);      // Set the AER Acknowledge Set Delay
...
//        icub_vsctrl_write_word(pdata, C_BG_PRESC_ADDR32, 125000);  // Set the BG Prescaler Value
        ...
        icub_vsctrl_write_byte(pdata, C_BGTIM2_ADDR, 1);           // Set the BG Clock Active Time
        icub_vsctrl_write_byte(pdata, C_BGTIM3_ADDR, 1);           // Set the BG Setup/Hold Time
        ...
        if (pdata->chip_type != chip_type) {
            ...
            icub_vsctrl_write_byte(pdata, C_BGCNFG0_ADDR, 0x39);  // BG clock and latch active high - Force the BG type to "SOFTWARE"
        } else {
        ...
        icub_vsctrl_write_byte(pdata, C_SRCTIM2_ADDR, 2);      // Set the AER Data Sample Delay  TBD
        icub_vsctrl_write_byte(pdata, C_SRCTIM3_ADDR, 3);      // Set the AER Acknowledge Release Delay TBD
        
        icub_vsctrl_write_word(pdata, C_BG_PRESC_ADDR32, 3);   // Set the BG Prescaler Value  TBD
        icub_vsctrl_write_byte(pdata, C_BGTIM0_ADDR, 50);      // Set the BG Latch Active Time  TBD
        icub_vsctrl_write_byte(pdata, C_BGTIM1_ADDR, 75);      // Set the BG Latch Setup Time  TBD
        icub_vsctrl_write_byte(pdata, C_BGTIM2_ADDR, 50);      // Set the BG Clock Active Time  TBD
        icub_vsctrl_write_byte(pdata, C_BGTIM3_ADDR, 25);      // Set the BG Setup/Hold Time  TBD
...
#else  // !defined(DVS_DONTSEND_LAST_12BIT
