...
#define DATA_STORAGE_U8  ((DATA_STORAGE_U32 << 2) + 1)
...
struct icub_vsctrl_data {
    ...
    uint8_t  wr_buf[DATA_STORAGE_U8];
    uint8_t  rd_buf[DATA_STORAGE_U8];
    uint32_t *data_write;
    uint8_t  *addr_write;
    uint32_t *data_read;
    uint8_t  *addr_read;
...
static int icub_vsctrl_fifowrite_byte(struct icub_vsctrl_data *pdata,
                                      uint8_t addr, uint8_t wrbuf[], u16 len)
{
    wrbuf[0] = (addr&0x3F)|0x00;
    return icub_vsctrl_readwrite(pdata->client, len+1, wrbuf, 0, NULL);
}
...
static int icub_vsctrl_fifowrite_word(struct icub_vsctrl_data *pdata,
                                      uint8_t addr, uint32_t wrbuf[], u16 len)
{
    wrbuf[0] = (addr&0x3F)|0x40;
    return icub_vsctrl_readwrite(pdata->client, 4*len+1, (uint8_t *)wrbuf, 0, NULL);
}
...
static int icub_vsctrl_fiforead_byte(struct icub_vsctrl_data *pdata,
                                     uint8_t addr, uint8_t rdbuf[], u16 len)
{
    uint8_t wrbuf[1] = {0};

    wrbuf[0] = (addr&0x3F)|0x00;
    
    return icub_vsctrl_readwrite(pdata->client, 1, wrbuf, len, rdbuf+1);
}
...
static int icub_vsctrl_fiforead_word(struct icub_vsctrl_data *pdata,
                                     uint8_t addr, uint32_t rdbuf[], u16 len)
{
    uint8_t wrbuf[1] = {0};

    wrbuf[0] = (addr&0x3F)|0x40;
    
    return icub_vsctrl_readwrite(pdata->client, 1, wrbuf, 4*len, ((uint8_t *)rdbuf)+1);
...
uint32_t icub_vsctrl_crc_check(uint32_t *data, int len, uint32_t poly) {
    return 0;
}
...
static int icub_vsctrl_misc_open (struct inode *ip, struct file *fp) {
    ...
    if (pdata->roi_notbg && pdata->crc_check_en)
        tmp_byte = (tmp_byte & ~C_BG_BIASROI_MSK ) | (C_BG_CRCCLR_MSK | C_BG_CRCEN_MSK);
    else
        tmp_byte = (tmp_byte | C_BG_BIASROI_MSK ) & ~C_BG_CRCEN_MSK;
...
static ssize_t icub_vsctrl_misc_write (struct file *fp, const char __user *buf, size_t len, loff_t *ppos) {
    ...
    if (pdata->chip_type == C_CHIP_DVS) {
        ...
        icub_vsctrl_write_word(pdata, C_BG_DATA_ADDR32, 0x00);
    }
...
static ssize_t icub_vsctrl_misc_read (struct file *fp, char __user *buf, size_t len, loff_t *ppos) {
    ...
    mutex_lock(&pdata->mutex);
    icub_vsctrl_fiforead_word(pdata, C_BG_DATA_ADDR32, pdata->data_read, length_uint);
...
static long icub_vsctrl_misc_ioctl (struct file *fp, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        ...
        case IOC_GET_STATUS:
            if (icub_vsctrl_read_word(pdata, C_INFO_ADDR32, &tmp_word))
                goto read_err;
...
static int icub_vsctrl_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    ...
    pdata->data_write = (uint32_t *) &(pdata->wr_buf[1]);
    pdata->addr_write = (uint8_t *)  &(pdata->wr_buf[0]);
    pdata->data_read  = (uint32_t *) &(pdata->rd_buf[1]);
    pdata->addr_read  = (uint8_t *)  &(pdata->rd_buf[0]);
